...
static py::tuple py_quantize_hc16(py::array_t<float> w) {
    auto buf = w.request();
...
static py::array_t<float> py_dequantize_hc16(py::array_t<int16_t> h, float scale) {
    auto buf = h.request();
...
static py::tuple py_quantize_hc8(py::array_t<float> w) {
    auto buf = w.request();
    if (buf.ndim != 1) throw std::runtime_error("w must be 1-D");
...
static py::array_t<float> py_dequantize_hc8(py::array_t<int16_t> h, float scale) {
    auto buf = h.request();
    if (buf.ndim != 1) throw std::runtime_error("h must be 1-D");
    ...
    py::array_t<float> out(2 * n);  /* 2 个 float/hc16ms_t */
    auto out_buf = out.request();
    float* out_ptr = (float*)out_buf.ptr;
...
static py::dict py_inspect_views(py::array_t<int16_t> h) {
...
static py::dict py_inspect_views_swapped(py::array_t<int16_t> h) {
...
static py::array_t<float> py_matmul_hc16(
        py::array_t<int16_t> a, py::array_t<int16_t> b,
    ...
    if (a_buf.size != (py::ssize_t)(m * k))
        throw std::runtime_error("a size mismatch");
    if (b_buf.size != (py::ssize_t)(k * n))
...
static py::array_t<float> py_quantized_matmul_hc16(
    ...
    if (x_buf.size != (py::ssize_t)(m * k))
        throw std::runtime_error("x size mismatch");
    if (w_buf.size != (py::ssize_t)(k * n))
