...
static std::vector<double> py_float_matmul(
    ...
    /* 转置 b 以便列访问变行访问（缓存友好） */
    /* b_t[j * k + l] = b[l * n + j]，即 b_t 是 b 的转置 */
    std::vector<double> b_t((size_t)n * k);
     ...
     * 使 C 计算可与 Python 线程重叠（阶段 0.2，multithreading_optimization_plan.md） */
    {
        ...
        for (uint32_t l = 0; l < k; ++l) {
            for (uint32_t j = 0; j < n; ++j) {
                b_t[(size_t)j * k + l] = b_flat[(size_t)l * n + j];
            }
        }
 ...
 *   4. float 累加到输出 y
 *
 * 与 Python sbe_conv2d.sbe_matmul 数学等价，但内部用 VNNI kernel 替代 numpy matmul。
 *
 * Args:
