...
static py::array_t<int16_t> make_int16_array(size_t n) {
    return py::array_t<int16_t>({(py::ssize_t)n}, {sizeof(int16_t)});
}

static const float* get_float_ptr(const py::object& obj, size_t expected_n) {
    auto arr = obj.cast<py::array_t<float, py::array::c_style | py::array::forcecast>>();
    auto buf = arr.request();
    if ((size_t)buf.size != expected_n) {
        throw std::runtime_error("数组长度不匹配：期望 " + std::to_string(expected_n) +
                                 "，实际 " + std::to_string(buf.size));
    }
    return (const float*)buf.ptr;
}

static const int16_t* get_int16_ptr(const py::object& obj, size_t expected_n) {
    auto arr = obj.cast<py::array_t<int16_t, py::array::c_style | py::array::forcecast>>();
    auto buf = arr.request();
    if ((size_t)buf.size != expected_n) {
        throw std::runtime_error("数组长度不匹配：期望 " + std::to_string(expected_n) +
                                 "，实际 " + std::to_string(buf.size));
    }
    return (const int16_t*)buf.ptr;
}
...
static float py_quant_compute_scale(const py::object& x_obj) {
    ...
    return hc16_quant_compute_scale((const float*)buf.ptr, (uint32_t)buf.size);
}
...
static py::array_t<float> py_matmul(const py::object& a_obj,
    ...
    const int16_t* a = get_int16_ptr(a_obj, (size_t)m * k);
    const int16_t* b = get_int16_ptr(b_obj, (size_t)k * n);
...
static py::array_t<float> py_quantized_matmul(const py::object& x_obj,
                                               ...
                                               uint32_t m, uint32_t k, uint32_t n,
                                               const PyQuantSchema16& schema) {
    const float* x = get_float_ptr(x_obj, (size_t)m * k);
    const float* w = get_float_ptr(w_obj, (size_t)k * n);
 ...
 *
 * 注意：不使用 get_int16_ptr（该函数返回的指针在 forcecast 副本销毁后悬空），
